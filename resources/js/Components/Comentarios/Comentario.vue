...
const getComentarios = async () => {
    const response = await axios.get(route('comentario.getComentarios', reporte_semanal_id));
                ...
                <div v-if="comentarios" class="my-4 h-60 overflow-y-auto">
                    <ul v-for="comentario in comentarios" :key="comentario.id" class="list-disc list-inside">
                        <div class="flex justify-between">
                            ...
                            <button class="float-right mx-4 pi pi-trash text-red-500
                            cursor-pointer hover:text-red-300" @click.stop="deleteComentario(comentario.id)"></button>
                        </div>
                    </ul>
                </div>
